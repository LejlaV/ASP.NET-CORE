...
namespace Ispit_2017_09_11_DotnetCore.EF
{
    public class MojDBInitializer
    {
        public static void Izvrsi(MojContext _context)
        {
            ...
            var nastavnik = new List<Nastavnik>();
            ...
            for (int i = 0; i < 40; i++)
            {
                //nastavnik.Add(new Nastavnik { ImePrezime = "Nastavnik " + Guid.NewGuid().ToString().Substring(0, 3) });
                _context.Nastavnik.Add(new Nastavnik { ImePrezime = "Nastavnik " + Guid.NewGuid().ToString().Substring(0, 3) });
